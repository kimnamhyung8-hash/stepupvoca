...
// ----------------------------------------------------------------------
// Fix Capacitor 8 SPM Compatibility for @capacitor-community/speech-recognition
// ----------------------------------------------------------------------
const pSwiftPath = path.join(__dirname, 'node_modules', '@capacitor-community', 'speech-recognition', 'Package.swift');
if (fs.existsSync(path.dirname(pSwiftPath)) && !fs.existsSync(pSwiftPath)) {
...
`;
    fs.writeFileSync(pSwiftPath, pkgSwiftContent, 'utf8');
    console.log('[postinstall] ✅ Created Package.swift for @capacitor-community/speech-recognition');
}
