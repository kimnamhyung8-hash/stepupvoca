...
`;
    ...
    const setupPeerConnection = async (roomId: string) => {
        ...
        pc.ontrack = (e) => {
            console.log('[WebRTC] Remote track received:', e.streams[0]?.id, e.track.kind);
        ...
        <div className="w-full flex-1 bg-[#0A0A0E] flex flex-col overflow-hidden font-sans sm:max-h-[720px] sm:rounded-[32px] sm:shadow-2xl">
            ...
            {gameState === 'CHAT' && (activeRoom || isAiMode) && (
                <div className="flex flex-col h-full bg-[#0A0A0E] relative z-0">
                    <header className="px-5 pt-4 pb-3 flex flex-col gap-3 shrink-0 border-b border-white/5 bg-[#12121A] z-20">
                        <div className="flex items-center justify-between">
